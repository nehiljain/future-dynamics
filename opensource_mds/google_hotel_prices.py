...
async def fetch_google_hotel_prices_desktop(hotel_name, checkin_date, length_of_stay=1) -> List[dict]:
    ...
    screenshot_filepath = Path(
        SCREENSHOTS_DPATH,
        f'desktop_full_page_screenshot__{sanitized_hotel_name}__{target_date.format("YYYYMMDD")}__{pendulum.now("UTC").format("YYYYMMDDHHmm")}.png',
    )
            ...
            try:
                ...
                if not is_right_page:
                    ...
                    if "View Prices".lower() in "".join(is_list_item_shown).lower():
                        ...
                        logger.debug(
                            "Clicking on view prices button to open hotel specific page"
                        )
                        await butt.click()
                logger.debug(
                ...
                df_filepath = screenshot_filepath.with_suffix(".parquet")
                df = pd.DataFrame([item for item in prices])
