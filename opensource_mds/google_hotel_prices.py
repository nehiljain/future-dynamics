...
from .constants import SCREENSHOTS_DPATH
import re
import pendulum
  ...
  async with semaphore:
    async with async_playwright() as playwright:
      ...
      try:
        ...
        screenshot_filepath = Path(SCREENSHOTS_DPATH, f'desktop_full_page_screenshot__{sanitized_hotel_name}__{dest_checkin_date.format("YYYYMMDD")}__{pendulum.now("UTC").format("YYYYMMDDHHmm")}.png')
...
async def fetch_google_hotel_prices_mobile(hotel_name, checkin_date, length_of_stay) -> List[dict]:
    logger.debug(f"Starting mobile price fetch for hotel: {hotel_name}, check-in: {checkin_date}, stay length: {length_of_stay} days")
    async with semaphore:
      async with async_playwright() as playwright:
        prices = []
        ...
        try:
            ...
            logger.debug("Searched on google for hotel %s and clicked on check availability", hotel_name)
            await asyncio.sleep(5)
            ...
            fin_checkin_date = pendulum.from_format(final_checkin_date, 'ddd, MMM D')
            fin_checkout_date = pendulum.from_format(final_checkout_date, 'ddd, MMM D')
            ...
            while not await page.get_by_role("button", name=re.compile("Fewer Options", re.IGNORECASE)).is_visible():
                await page.get_by_role("button", name=re.compile("View more options from \$\d+", re.IGNORECASE)).first.click()
                await asyncio.sleep(10)

            sanitized_hotel_name = re.sub(r'[^A-Za-z0-9]', '-', hotel_name)
            screenshot_filepath = Path(SCREENSHOTS_DPATH, f'mobile_full_page_screenshot__{sanitized_hotel_name}__{target_date.format("YYYYMMDD")}__{pendulum.now("UTC").format("YYYYMMDDHHmm")}.png')
            logger.info(f"Saving screenshot to {screenshot_filepath}")
            await page.screenshot(path=screenshot_filepath, full_page=True)
            ...
            for price in all_prices:
                ...
                text = re.sub(re.escape('View site'), '', text[0], flags=re.IGNORECASE)
                text = text.replace('\n',';;')
                price_obj = Price(text=text+";;mobile", hotel_name=hotel_name, checkin_date=checkin_date, length_of_stay=length_of_stay, scrapped_url=page.url)
                logger.debug("Price object: %s", dict(price_obj))
                prices.append(dict(price_obj))
            await context.close()
            ...
            logger.error(f"Error during mobile price scraping for hotel {hotel_name} with checkin date {checkin_date} and length of stay {length_of_stay}: {e}")
            price_obj = Price(text=f'Failed to load page {e}', hotel_name=hotel_name, checkin_date=checkin_date, length_of_stay=length_of_stay, scrapped_url=page.url)
            logger.debug("Price object: %s", dict(price_obj))
            prices.append(dict(price_obj))
...
async def execute_price_scraping_workflow(inputs):
  results = await asyncio.gather(*(fetch_google_hotel_prices_desktop(hotel_name="chelsea hotel, toronto", checkin_date="2024-04-12", length_of_stay=1) for i in range(1)))
  return results
